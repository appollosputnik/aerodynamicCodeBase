...
void Counter::setValue(int value)
{
    ...
    float N = 100;
    float A = 50;
    float CL = N * cos(alpha) - A * sin(alpha);
    d_counter->setValue( CL );
}
...
QToolBar *MainWindow_::toolBar()
{
    ...
    d_randomCount = new Counter( hBox, "Points", QString(), 1, 100000, 1 );
...
void MainWindow_::appendPoints( bool on )
{
    if ( on )
...
void MainWindow_::showRunning( bool running )
{
...
void MainWindow_::showElapsed( int ms )
{
    QString text;
    text.setNum( ms );
    text += " ms";
...
void MainWindow_::initWhatsThis()
{
    const char *text1 =
